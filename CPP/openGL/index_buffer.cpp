...
"}\0";
const char *fragmentShaderSource = "#version 330 cor\n"
"out vec4 FragColor;\n"
"void main()\n"
"{\n"
...
int main() {
    ...
    GLfloat vertices[]=
    {
        -0.5f, -0.5f * float(sqrt(3))/ 3, 0.3f,//esquerda inferior
        0.5f, -0.5f * float(sqrt(3))/ 3, 0.0f,//esquerda superior
        0.0f, 0.5f * float(sqrt(3))/ 2, 0.0f,//topo
        -0.05f / 2, 0.5f * float(sqrt(3))/6,0.0f,//esquerda interna
        0.05f / 2, 0.5f * float(sqrt(3))/6,0.0f,//direita interna
        0.0f, -0.5f * float(sqrt(3))/3,0.0f,//baixo interno
    };
    GLuint indices[] ={
        0,3,5,//esquerda inferior do triangulo
        3,2,4,//direita inferior do triangulo
        5,4,1//topo do triangulo
    };
    ...
    glGenBuffers(1,&EBO);//valor de referencia do index buffer
    glBindBuffer(GL_ARRAY_BUFFER, VBO);//tipo de gl buffer
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices),indices, GL_STATIC_DRAW);//conectando com o array de indices
